...
반복적으로 맡길 작업에서는 검토에 드는 시간과 결과의 정확성을 함께 살핍니다. 사람과 AI를 결합한 성능이 더 나은 단독 수행을 넘는지는 작업에 따라 다르므로, 사람이 승인했다는 사실과 작업의 성능을 구분합니다.[^human-ai]
...
같은 질문을 AI에 다시 보내는 것만으로 계산을 재현했다고 보지 않습니다. 자료와 계산의 기록 및 재현 조건은 [판단과 근거](APPROACH.md#계산과-판단-기록)을 따릅니다.
