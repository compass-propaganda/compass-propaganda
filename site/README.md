...
권장의 효력과 대체 관계는 [운영과 참여](../GOVERNANCE.md#제안에서-발행까지)의 원문 표기를 읽어 인덱스에도 포함합니다. 홈과 현행 목록에는 현행 권장만 표시하고, 철회·대체된 권장은 별도로 남깁니다. 효력 표시 누락·중복, 대체 대상 누락과 순환 참조는 빌드 오류로 처리합니다.

## 검색과 공유 메타데이터

페이지마다 제목·설명·canonical, [Open Graph](https://ogp.me/)와 공유 카드 메타데이터를 생성합니다. 설명은 원문의 첫 문단을 사용하며, 권장은 권장 요지를 사용합니다. 홈·판단 원칙에는 목적을, 용어·목록·다운로드에는 해당 페이지의 소개를 사용합니다.
